 ...
 */
export function parseMarkdown(markdown: string, warnings: string[]): DocxBlock[] {
  ...
  let list: { ordered: boolean; items: Array<{ level: number; text: string }> } | null = null
  ...
  while (index < lines.length) {
    ...
    if (line.trim().startsWith('|') && next !== undefined && TABLE_SEPARATOR.test(next.trim())) {
      ...
      while (index < lines.length) {
        ...
        if (rowLine === undefined || !rowLine.trim().startsWith('|')) break
    ...
    if (heading) {
      ...
      if (hashes !== undefined && text !== undefined) {
        blocks.push({ kind: 'heading', level: hashes.length as 1 | 2 | 3 | 4 | 5 | 6, text: text.trim() })
      }
      index += 1
      continue
    }
