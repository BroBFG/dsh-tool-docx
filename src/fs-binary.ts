 ...
 */
export function resolveWriteBytes(ctx: Context): FsBytesWriter | undefined {
  const fsBinary = ctx.get(FS_BINARY_SERVICE) as { writeBytes?: FsBytesWriter } | undefined
  if (fsBinary?.writeBytes !== undefined) return fsBinary.writeBytes.bind(fsBinary)
  const fs = ctx.fs as Partial<BinaryFileSystem>
 ...
 */
export function requireWriteBytes(ctx: Context): FsBytesWriter {
  const writeBytes = resolveWriteBytes(ctx)
  if (writeBytes === undefined) {
    throw new DocxError(
      'the host filesystem seam lacks the binary writeBytes primitive — install the dsh-tool-docx bundle'
      + ` (dsh plugin --profile web add dsh-tool-docx), which mounts the ${FS_BINARY_SERVICE} provider,`
      + ' or use a deepseek-harness build that includes fs.writeBytes natively',
