...
import { writableRoots, type SandboxExecutionPolicy } from '@deepseek-ai/dsh-sandbox'
import type { FsBytesWriteOutcome } from './fs-binary.ts'
import { performByteWrite } from './fs-binary-local.ts'
import { isPathUnder } from './path-contains.ts'
...
export async function checkWriteTarget(
  resolveTarget: (displayPath: string) => Promise<FsTarget>,
...
export class DocxSandboxedFileSystem extends SandboxedFileSystem {
   ...
   */
  private async checkedWriteTarget(target: FsTarget, sandboxPolicy?: SandboxExecutionPolicy): Promise<FsTarget> {
